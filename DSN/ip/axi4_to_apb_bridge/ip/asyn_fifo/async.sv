...
module asynchronous_fifo #(parameter DEPTH=8, DATA_WIDTH= 128) (
  input wclk, wrst_n,
  input rclk, rrst_n,
  input w_en, r_en,
  input [DATA_WIDTH-1:0] data_in,
  //output reg [DATA_WIDTH-1:0] data_out,cahnged
  ...
  fifo_mem #(.DATA_WIDTH(DATA_WIDTH),.PTR_WIDTH(PTR_WIDTH),.DEPTH(DEPTH)) fifom(wclk, w_en, rclk, r_en,b_wptr, b_rptr, data_in,full,empty, data_out);

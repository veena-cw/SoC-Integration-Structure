...
(
  ...
  /* -- slave-side ports commented out --
    ...
    if (!HRESETn) access_state <= NO_ACCESS;
      ...
      case (access_state)
        ...
        ACCESS_PENDING: if ( |(pending_HSEL & master_granted)  &&
                             /* slvHREADY[slave_sel] -- slave-side, commented out */ 1'b1 ) access_state <= ACCESS_GRANTED;

        ACCESS_GRANTED: if      (mst_HREADY && ~|current_HSEL                                ) access_state <= NO_ACCESS;
                        else if (mst_HREADY && ~|(current_HSEL & master_granted /* & slvHREADY -- slave-side, commented out */) ) access_state <= ACCESS_PENDING;
   ...
   */
  /* -- slave-request decode / fan-out generate block commented out --
generate
  for (s=0; s<SLAVES; s++)
  begin: gen_HSEL
...
endgenerate
  -- end slave-request decode / fan-out generate block -- */
  ...
  always @(posedge HCLK,negedge HRESETn)
    ...
    else if ( mst_HREADY  ) slave_sel <= onehot2int( /* slvHSEL -- slave-side, commented out */ current_HSEL );

  /*
   * Outgoing data (to slaves)
   */
  /* -- outgoing-to-slaves assigns commented out --
  ...
  -- end outgoing-to-slaves assigns -- */


  /*
   * Incoming data (to masters)
   */
  /* -- incoming-from-slaves assigns commented out --
  ...
  -- end incoming-from-slaves assigns -- */
